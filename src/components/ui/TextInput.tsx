      ...
      <div className='flex flex-col gap-2'>
        {label && (
          <label htmlFor={id} className='semibold-3 text-neutral-12'>
            {label}
            {labelExtension && (
              <span className='text-neutral-11 medium-3 ml-0.5'>
                {labelExtension}
              </span>
            )}
            {optional && (
        ...
        <input
          ref={ref}
          className={clsx(
            'relative placeholder:text-[14px] placeholder:leading-5 hover:bg-panel-3 bg-panel-1 form-input h-12 rounded-[10px] flex items-center px-3 gap-2 text-[14px] leading-5 border border-neutral-alpha-6 placeholder-neutral-alpha-11 focus:input-focus-default  text-neutral-alpha-12',
